"""Wan2.1-T2V-14B + TeaCache (TaylorSeerCacheConfig). Published 1.6-2× on Wan family."""
...
def run_wan_teacache(cache_interval: int = 5, max_order: int = 1,
    ...
    try:
        ...
        print(f"  TaylorSeerCacheConfig not in diffusers: {exc}")
    ...
    return {"model": REPO, "kernel": "teacache_taylorseer", "cache_enabled": cache_enabled,
            "cache_interval": cache_interval, "max_order": max_order,
            "disable_cache_before_step": disable_cache_before_step,
...
@app.local_entrypoint(name="wan21_14b_teacache_modal")
def main(num_iters: int = 1):
    r = run_wan_teacache.remote(num_iters=num_iters)
    import json; print(json.dumps(r, indent=2))
    sys.path.insert(0, "/root/benchmarks")
    from result_store import save_result
    save_result(r, "optimised_kernels", "wan21_14b_teacache")
