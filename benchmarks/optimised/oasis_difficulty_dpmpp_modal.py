"""
Modal H100: Difficulty-Aware Step Budget + DPM-Solver++ 2M (5 base steps).

Stacks two orthogonal optimizations:
  * DPM-Solver++ 2M cuts the per-frame ceiling from 10 → 5 (independent ~2x)
  * Difficulty schedule further cuts within those 5 based on action magnitude

Per-frame steps:
    idle action (mag < lo):   reduced (e.g. 2 of 5)
    moderate (lo <= mag < hi): mid     (e.g. 3 of 5)
    high motion (mag >= hi):   full    (5)

Hypothesis: ~3.5x speedup with preserved quality, since both optimizations
attack step count from different angles (multistep solver vs adaptive budget).
"""
...
def run_oasis_difficulty_dpmpp(
    num_frames: int = 32,
    n_prompt: int = 1,
    ddim_steps: int = 10,                 # for baseline pass
    solver_full_steps: int = 5,           # DPM-Solver++ ceiling
    ...
    def _generate(use_stack: bool, seed_val: int = 42) -> torch.Tensor:
        ...
        for i in range(n_prompt, total_frames):
            if use_stack:
                # Difficulty-aware step count (within DPM-Solver++ ceiling)
                steps_i = steps_for_frame(i)
            else:
                steps_i = ddim_steps
    ...
    print(f"\n=== Pass 2: DPM-Solver++ {solver_full_steps}/{solver_mid_steps}/{solver_reduced_steps} + Difficulty ===")
    sk_lat, sk_ms = _time_run(lambda: _generate(use_stack=True, seed_val=seed))
    print(f"Stack: {sk_ms:.1f} ms")
    ...
    def _save(pix, fname):
        ...
        return path
    bl_mp4 = _save(bl_pix, f"baseline_diff_dpmpp_f{num_frames}_seed{seed}.mp4")
    sk_mp4 = _save(sc_pix, f"diff_dpmpp_r{solver_reduced_steps}m{solver_mid_steps}f{solver_full_steps}_f{num_frames}_seed{seed}.mp4")
    sxs_mp4 = os.path.join(out_dir, f"sidebyside_diff_dpmpp_f{num_frames}_seed{seed}.mp4")
    try:
    ...
    return {
        ...
        "kernel": f"diff_dpmpp_r{solver_reduced_steps}m{solver_mid_steps}f{solver_full_steps}",
        "thresholds": {"lo": lo_threshold, "hi": hi_threshold,
                       "reduced": solver_reduced_steps, "mid": solver_mid_steps, "full": solver_full_steps},
        "baseline_latency_ms": round(bl_ms, 2),
        "stack_latency_ms": round(sk_ms, 2),
        "speedup": round(bl_ms / sk_ms, 3),
        "quality_aggregate": aggregate,
        "self_coherence_baseline": coh_bl,
        "self_coherence_stack": coh_sk,
        "videos": {"baseline_mp4": bl_mp4, "stack_mp4": sk_mp4, "sidebyside_mp4": sxs_mp4},
        "num_frames": num_frames, "seed": seed, "gpu": "H100",
    }


@app.local_entrypoint(name="oasis_difficulty_dpmpp_modal")
def main(num_frames: int = 32, seed: int = 42,
         solver_full_steps: int = 5, solver_mid_steps: int = 3, solver_reduced_steps: int = 2):
    result = run_oasis_difficulty_dpmpp.remote(
    ...
    print(f"\n=== Difficulty + DPM-Solver++ stacked ===")
    print(f"  speedup: {result.get('speedup')}x")
    print(f"  vs_prev: base={result['self_coherence_baseline']['vs_prev_psnr_mean']} stack={result['self_coherence_stack']['vs_prev_psnr_mean']}")
    print(f"  cross-PSNR mean: {result['quality_aggregate']['psnr_mean_db']} dB")
    sys.path.insert(0, "/root/benchmarks")
    from result_store import save_result
    save_result(result, "optimised_kernels", f"oasis_diff_dpmpp_r{solver_reduced_steps}m{solver_mid_steps}_f{num_frames}")
