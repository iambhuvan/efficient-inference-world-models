"""CogVideoX-5B + SageAttention2 (after image rebuild with fixed sageattn 2.x source build).
Probes backends in order: SM90 INT4-Q/FP8-PV → FA3 → native SDPA. Expected 1.5× if Sage2 callable."""
...
def run_cogvideox_sageattn(num_warmup: int = 1, num_iters: int = 1) -> dict:
    import os, statistics, torch
    from huggingface_hub import snapshot_download

    # ── Probe attention backends ─────────────────────────────────────────
    ...
    return {"model": REPO, "kernel": "sageattn_or_fallback",
            "backend_used": chosen, "candidates": candidates,
            "n_attention_modules_patched": n_patched, "num_frames_observed": n,
...
@app.local_entrypoint(name="cogvideox_5b_sageattn_modal")
def main(num_iters: int = 1):
    r = run_cogvideox_sageattn.remote(num_iters=num_iters)
    import json; print(json.dumps(r, indent=2))
    sys.path.insert(0, "/root/benchmarks")
    from result_store import save_result
    save_result(r, "optimised_kernels", "cogvideox_5b_sageattn")
