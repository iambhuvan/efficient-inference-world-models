"""
Modal H100 quality eval: baseline vs step-cache on the SAME seed + real Oasis data.
Reports per-frame PSNR vs baseline, KVCache-style self-coherence, and saves three
MP4s (baseline, stepcache, side-by-side) to the volume.

The stepcache benchmark gave 2.22x speedup at skip_pattern=alternate (50% v reuse).
This eval determines whether that 50% reuse degrades output quality at all.
"""
...
def _build_compute_set(pattern: str, ddim_steps: int) -> set:
    ...
    if pattern == "skip_late":
        return set(range(ddim_steps, ddim_steps // 2, -1))
    if pattern == "skip_early":
        return set(range(ddim_steps // 2, 0, -1))
...
def run_oasis_stepcache_quality(
    num_frames: int = 32,
    n_prompt: int = 1,
    ddim_steps: int = 10,
    ...
    print("Downloading + loading Oasis DiT/VAE ...")
    ...
    print(f"\nskip_pattern={skip_pattern!r}, computing on noise_idx {sorted(compute_steps)}")

    def _generate(use_stepcache: bool, seed_val: int = 42) -> torch.Tensor:
        ...
        for i in range(n_prompt, total_frames):
            ...
            for noise_idx in reversed(range(1, ddim_steps + 1)):
                ...
                # Step-cache logic only enabled in second pass
                if use_stepcache and (noise_idx not in compute_steps) and (cached_v is not None):
                    v = cached_v  # reuse
                else:
                    with torch.no_grad():
                        with torch.cuda.amp.autocast(dtype=torch.float16):
    ...
    s.record()
    baseline_lat = _generate(use_stepcache=False, seed_val=seed)
    ...
    s = torch.cuda.Event(enable_timing=True); e = torch.cuda.Event(enable_timing=True)
    s.record()
    stepcache_lat = _generate(use_stepcache=True, seed_val=seed)
    e.record(); torch.cuda.synchronize()
    stepcache_ms = s.elapsed_time(e)
    print(f"Stepcache: {stepcache_ms:.1f} ms ({num_frames / (stepcache_ms / 1000):.2f} fps)")
    ...
    sc_pix = _decode(stepcache_lat.float()).cpu()

    # Per-frame divergence (cross-PSNR)
    print("\n=== Per-frame divergence ===")
    per_frame: list = []
    bl_lat = baseline_lat[0].float().cpu()
    sc_lat = stepcache_lat[0].float().cpu()
    ...
    coh_stepcache = _self_coh(sc_pix)
    print(f"\nSelf-coherence:")
    print(f"  baseline:   {coh_baseline}")
    print(f"  stepcache:  {coh_stepcache}")
    ...
    bl_mp4 = _save(bl_pix, f"baseline_stepcache_f{num_frames}_seed{seed}.mp4")
    sc_mp4 = _save(sc_pix, f"stepcache_{skip_pattern}_f{num_frames}_seed{seed}.mp4")
    sxs_mp4 = os.path.join(out_dir, f"sidebyside_stepcache_{skip_pattern}_f{num_frames}_seed{seed}.mp4")
    try:
    ...
    speedup = baseline_ms / stepcache_ms
    return {
        "model": "Etched/oasis-500m",
        "kernel": f"stepcache_quality_eval_{skip_pattern}",
        "skip_pattern": skip_pattern,
        "computed_steps_per_frame": sorted(compute_steps),
        "baseline_latency_ms": round(baseline_ms, 2),
        "stepcache_latency_ms": round(stepcache_ms, 2),
        "speedup": round(speedup, 3),
        "baseline_fps": round(num_frames / (baseline_ms / 1000), 3),
        "stepcache_fps": round(num_frames / (stepcache_ms / 1000), 3),
        "quality_aggregate": aggregate,
        "self_coherence_baseline": coh_baseline,
        "self_coherence_stepcache": coh_stepcache,
        "videos": {"baseline_mp4": bl_mp4, "stepcache_mp4": sc_mp4, "sidebyside_mp4": sxs_mp4},
...
@app.local_entrypoint(name="oasis_stepcache_quality_modal")
def main(num_frames: int = 32, seed: int = 42, skip_pattern: str = "alternate"):
    result = run_oasis_stepcache_quality.remote(
        num_frames=num_frames, seed=seed, skip_pattern=skip_pattern,
    )
    print(f"\n=== Step cache quality eval ===")
    ...
    print(f"  vs_prev (sc):   {result['self_coherence_stepcache']['vs_prev_psnr_mean']} dB")
    sys.path.insert(0, "/root/benchmarks")
    from result_store import save_result
    save_result(result, "optimised_kernels", f"oasis_stepcache_quality_{skip_pattern}_f{num_frames}")
